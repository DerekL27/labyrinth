 ...
 * - Rotates the spare tile counterclockwise (optional)
...
public class BasicTurnAction implements Action {
  ...
  @Override
  public Tile accept(Board board, Tile spare, List<PlayerData> players, List<Posn> additionalGoals) {
    ...
    currentPlayer = currentPlayer.updateIfReachedTarget(additionalGoals);
  ...
  private boolean willUndoPreviousMove(Optional<Tuple<Integer, Direction>> prevMove) {
    ...
    return currentPlan.getFirst().equals(prevMove.get().getFirst())
